...
fragment half4 chromaHaloRimFragment(
    ...
    constant uint   &coronaMode [[buffer(3)]]
) {
    constexpr sampler s(coord::normalized, address::clamp_to_edge, filter::linear);
    ...
    float emissionMul = (coronaMode != 0u) ? 2.85f : 2.18f;
    color.rgb *= half(intensity * emissionMul);
    ...
    // Reactive 2 corona: full lateral wings + wider vertical wash (emanates from all bezels).
    const float sideTight   = (coronaMode != 0u) ? 1.0f : (1.0f / 0.7f);
    float nxEff             = saturate(bx / max(capSafe / sideTight, 1e-4));
    const float tbTight     = (coronaMode != 0u) ? 1.05f : 1.42f;
