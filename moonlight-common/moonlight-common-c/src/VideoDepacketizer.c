...
void initializeVideoDepacketizer(int pktSize) {
    // Increased from 15 to 60 to handle initial decoder warmup on first launch
    // When VideoToolbox is initializing for the first time, frame decoding can be slow
    // and we need a larger buffer to avoid overflow during this period
    LbqInitializeLinkedBlockingQueue(&decodeUnitQueue, 60);
