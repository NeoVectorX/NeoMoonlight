...
- (void)reinitializeDisplayLayer
{
    ...
    // CRITICAL: Enable EDR (Extended Dynamic Range) for proper HDR display
    displayLayer.wantsExtendedDynamicRangeContent = YES;
    _view.layer.wantsExtendedDynamicRangeContent = YES;
    
    // On visionOS, we may also need to enable EDR on the window's layer
#if TARGET_OS_VISION
    if (_view.window) {
        _view.window.rootViewController.view.layer.wantsExtendedDynamicRangeContent = YES;
        Log(LOG_I, @"Enabled EDR on window root view layer");
    }
#endif
    ...
    Log(LOG_I, @"EDR Configuration - Display Layer: %d, Parent Layer: %d", 
        displayLayer.wantsExtendedDynamicRangeContent, _view.layer.wantsExtendedDynamicRangeContent);
}
    ...
    if (du->frameType == FRAME_TYPE_IDR) {
        ...
        if (videoFormat & VIDEO_FORMAT_MASK_H264) {
            ...
            if (contentLightLevelInfo) {
                ...
                Log(LOG_I, @"Added ContentLightLevelInfo to format description");
            }
            ...
            if (masteringDisplayColorVolume) {
                ...
                Log(LOG_I, @"Added MasteringDisplayColorVolume to format description");
            }
            ...
            if (status != noErr) {
                ...
                // Log the transfer function from the format description
                CFDictionaryRef extensions = CMFormatDescriptionGetExtensions(formatDesc);
                if (extensions) {
                    CFStringRef transferFunction = CFDictionaryGetValue(extensions, kCMFormatDescriptionExtension_TransferFunction);
                    CFStringRef colorPrimaries = CFDictionaryGetValue(extensions, kCMFormatDescriptionExtension_ColorPrimaries);
                    
                    Log(LOG_I, @"HEVC Format Description created successfully");
                    if (transferFunction) {
                        Log(LOG_I, @"Transfer Function: %@", (__bridge NSString*)transferFunction);
                    } else {
                        Log(LOG_W, @"No Transfer Function specified in format description!");
                    }
                    if (colorPrimaries) {
                        Log(LOG_I, @"Color Primaries: %@", (__bridge NSString*)colorPrimaries);
                    }
                }
            }
...
- (void)setHdrMode:(BOOL)enabled {
    ...
    Log(LOG_I, @"setHdrMode called with enabled=%d, hasMetadata=%d", enabled, hasMetadata);
    ...
    if (hasMetadata && hdrMetadata.displayPrimaries[0].x != 0 && hdrMetadata.maxDisplayLuminance != 0) {
        ...
        Log(LOG_I, @"HDR Display Metadata: maxLuminance=%d nits, minLuminance=%d", 
            hdrMetadata.maxDisplayLuminance, hdrMetadata.minDisplayLuminance);
    }
    else if (masteringDisplayColorVolume != nil) {
        masteringDisplayColorVolume = nil;
        metadataChanged = YES;
        Log(LOG_I, @"HDR disabled, clearing mastering display metadata");
    }
    ...
    if (hasMetadata && hdrMetadata.maxContentLightLevel != 0 && hdrMetadata.maxFrameAverageLightLevel != 0) {
        ...
        Log(LOG_I, @"HDR Content Light Level: maxCLL=%d nits, maxFALL=%d nits", 
    ...
    if (metadataChanged) {
        Log(LOG_I, @"HDR metadata changed, requesting IDR frame to update format description");
        
        // Also re-enable EDR on the layers when HDR is toggled
        displayLayer.wantsExtendedDynamicRangeContent = enabled;
        _view.layer.wantsExtendedDynamicRangeContent = enabled;
