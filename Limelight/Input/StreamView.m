...
- (void) setupStreamView:(ControllerSupport*)controllerSupport
    ...
    self->hideSystemCursor = settings.hideSystemCursor;
...
- (void) layoutSublayersOfLayer:(CALayer* ) layer {
    for (CALayer* sublayer in self.layer.sublayers) {
        if ([sublayer isKindOfClass:[AVSampleBufferDisplayLayer class]]) {
            ...
            sublayer.bounds = CGRectMake(0, 0, videoSize.width, videoSize.height);
            [CATransaction commit];
        }
        else if ([sublayer isKindOfClass:[CAMetalLayer class]]) {
            // Handle Metal renderer (Enhanced HDR 2D mode) - fill entire view
            // Window aspect ratio lock ensures correct proportions
            [CATransaction begin];
            [CATransaction setValue:(id)kCFBooleanTrue forKey:kCATransactionDisableActions];
...
- (UIPointerStyle *)pointerInteraction:(UIPointerInteraction *)interaction styleForRegion:(UIPointerRegion *)region  API_AVAILABLE(ios(13.4)) {
    ...
    if (hideSystemCursor) {
        ...
        // Return default style to show the system cursor (looking dot)
        return nil;
    }
}
